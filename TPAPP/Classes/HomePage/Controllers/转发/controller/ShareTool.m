...
- (void)performActivity
{
    if ([self.type isEqualToString:ActivityServiceWeixin]) {
        ...
        for (int i =0; i<self.imagedata.count; i++) {
            ShareItem *item = self.imagedata[i];
            
            UIImageWriteToSavedPhotosAlbum(item.img, self, @selector(image:didFinishSavingWithError:contextInfo:), nil);
        }
...
- (void)shareWithItems:(NSMutableArray*)items completionHandler:(UIActivityViewControllerCompletionHandler)completionHandler{
    ...
    [weixinFriendsActivity.imagedata addObjectsFromArray:items];
    ...
    [activities addObjectsFromArray:@[weixinFriendsActivity]];
