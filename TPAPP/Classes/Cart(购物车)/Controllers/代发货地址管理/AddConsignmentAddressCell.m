...
@interface AddConsignmentAddressCell ()<UITextViewDelegate,UITextFieldDelegate>
@end
@implementation AddConsignmentAddressCell
...
- (void)configWithModel:(NSMutableArray *)arr withModelData:(NSMutableArray *)modelArr withNumber:(NSInteger)num
{
    ...
    if ([arr[2] intValue] == 0) {
        ...
        self.myBlock([NSString stringWithFormat:@"%d",[self.defaultSwitch isOn]]);
...
- (void)switchAction:(id)sender
{
    ...
    self.myBlock([NSString stringWithFormat:@"%d",isButtonOn]);
    if (isButtonOn) {
    }else {
    }
}
-(void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    self.myBlock(self.myTextField.text);
